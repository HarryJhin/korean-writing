...
export const S2_MARKERS = [
  ...
  // 연결어미 뒤 쉼표(KatFishNet ACL 2025, 에세이 인간 4.10% vs LLM 19.83%).
  // 축약형 '해서'만 별도 등재 — '-서' 단독은 처소격 '에서,'를 오검출한다.
  { id: 'connectiveComma', regex: /[가-힣](고|며|지만|면서|는데|아서|어서|해서),/g },
  // have 직역(김정우 2007·이근희 2005).
  { id: 'haveAux', regex: /가지고\s*있/g },
  // 이중 조사(김정우 2007). 단순 '-의'와 '스스로의'류는 비대상 — 긴 대안을 앞에 둔다.
  { id: 'doubleParticle', regex: /(으로부터|로부터|에서|에로|으로|에)의/g },
  // 무맥락 결산 상투구. '따라서/그러므로'는 기술 문서 정당 빈출이라 제외.
  { id: 'conclusive', regex: /(결론적으로|요약하(?:자면|면)|정리하(?:자면|면)|종합하(?:자면|면))/g },
]
